...
class ShaderProgram {
    ...
    init(vertexShaderName: String, fragmentShaderName: String) {
        ...
        if linkStatus == GL_FALSE {
            ...
            DDLogError("Could not link shader file \(vertexShaderName) and \(fragmentShaderName): \(String(validatingUTF8: info) ?? "")")
            exit(1)
        }
    ...
    private func compileShader(name: String, with type: GLenum) -> GLuint {
        do {
            ...
            if compileStatus == GL_FALSE {
                ...
                DDLogError("Could not compile shader file \(name): \(String(validatingUTF8: info) ?? "")")
                exit(1)
            }
