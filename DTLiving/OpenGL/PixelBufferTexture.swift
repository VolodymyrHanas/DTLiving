...
class PixelBufferTexture {
    ...
    init(logTag: String, width: Int, height: Int, retainedBufferCountHint: Int = 0) {
        self.logTag = logTag
        self.width = width
        self.height = height
    ...
    func createTexture(from pixelBuffer: CVPixelBuffer) {
        ...
        if resultCode != kCVReturnSuccess {
            DDLogError("\(logTag) Could not create texture from image \(resultCode)")
            exit(1)
        }
    }
    ...
    func createPixelBuffer() -> CVPixelBuffer {
        ...
        if resultCode != kCVReturnSuccess {
            if resultCode == kCVReturnWouldExceedAllocationThreshold {
                DDLogError("\(logTag) Pool is out of buffers, dropping frame \(resultCode)")
            } else {
                DDLogError("\(logTag) Could not create pixel buffer in pool \(resultCode)")
            }
            exit(1)
        }
        return pixelBuffer
    ...
    func createTextureCache(in context: EAGLContext) {
        ...
        if resultCode != kCVReturnSuccess {
            DDLogError("\(logTag) Could not create texture cache \(resultCode)")
            exit(1)
        }
    }
    ...
    func createBufferPool() {
        ...
        let resultCode = CVPixelBufferPoolCreate(kCFAllocatorDefault,
                                                 ...
                                                 &bufferPool)
        if resultCode != kCVReturnSuccess {
            DDLogError("\(logTag) Could not create pixel buffer pool \(resultCode)")
            exit(1)
        }
